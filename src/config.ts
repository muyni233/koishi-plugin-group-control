...
export const Config: Schema<Config> = Schema.intersect([
    ...
    Schema.object({
        invite: Schema.object({
            ...
            inviteWaitMessage: Schema.string().role('textarea').default('已收到您的群聊邀请，正在等待管理员审核，请耐心等待。').description('发给邀请者的等待提示'),
            inviteApproveMessage: Schema.string().role('textarea').default('已自动通过您的群聊邀请，机器人正在加入群聊。').description('自动同意时发给邀请者的提示，支持变量 {groupName}, {groupId}, {userName}, {userId}'),
            inviteRequestMessage: Schema.string().role('textarea').default('收到新的群聊邀请请求：\n群名称：{groupName}\n群号：{groupId}\n邀请者：{userName} (QQ: {userId})\n\n同意请 gc.approve {groupId}，拒绝请 gc.reject {groupId}；也可引用本条消息后直接发送 gc.approve 或 gc.reject。').description('发给管理员的请求消息模板，支持变量 {groupName}, {groupId}, {userName}, {userId}'),
    ...
    Schema.object({
        friend: Schema.object({
            ...
            requestMessage: Schema.string().role('textarea').default('收到新的好友申请\nQQ：{userId}\n昵称：{nickname}\n附言：{comment}\n\n同意请 gc.approve {userId}，拒绝请 gc.reject {userId}；也可引用本条消息后直接发送 gc.approve 或 gc.reject。').description('通知管理员的消息模板，支持变量 {userId}, {nickname}, {comment}'),
