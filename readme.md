...
#### 目标解析（approve / reject / ban / unban / del 通用）
...
3. **自动识别**：approve/reject 对裸号会从待处理请求里自动判断是群还是好友；del 对裸号自动识别是好友还是所在群（两者都是则询问）；ban/unban 的裸号默认按**群**处理。
...
| `gc.del [目标]` | 删除好友 / 退出指定群（裸号默认退群，好友用 `friend:` 前缀或引用好友申请） |
