...
- **小群自管理**：
  ...
  - **实时监控**：监听群成员退群事件，当未经审核加入的群聊真人数缩水至阈值以下时，自动退群。
...
| `admin.deputyAdmins` | `[]` | **副管理员** QQ 号列表（可以使用 `gc` 指令，但在群内使用管理指令时仍会校验群管理员身份） |
| `admin.notificationGroupId` | *(空)* | **通知群号**（若填写，所有审核与事件通知发到此群，否则私聊发送至首个主管理员） |
...
> [!TIP]
> `gc` 系列管理指令对于主、副管理员均可用；主管理员额外拥有在群聊中绕过 `bot-on`/`bot-off`/`quit` 等指令的群管理员身份校验的特权。
...
| **实时小群监控** | | |
| `basic.smallGroupRealtimeMonitor` | `true` | 是否监听群员退群事件，当人数减少至阈值以下时自动退出（**仅监控未经审核的群**） |
...
**模板变量说明**：所有文案共享同一套变量 `{groupId}`/`{groupName}`（群号/群名）、`{userId}`/`{userName}`/`{nickname}`（用户号/名称，`userName` 与 `nickname` 相同）、`{comment}`（好友申请附言）、`{memberCount}`/`{threshold}`（小群当前人数/退群阈值）、`{duration}`（禁言时长秒数）、`{time}`（屏蔽剩余秒数）。

> [!WARNING]
> **引用消息解析机制**：
> 当管理员通过**回复引用机器人发的通知消息**来执行 `gc.approve` / `gc.ban` 等指令时，插件是通过匹配通知文案中的 **`群号：`**（用于群）或 **`好友申请` + `QQ：`**（用于好友）字样来识别目标 ID 的。
> **因此，在自定义管理员通知文案（如 `inviteRequestMessage`、`friendRequestMessage`）时，请务必在文案中保留这些关键中文字样！**
...
- **小群检测与豁免**：
  - 小群自管理只监控**未经审核直接拉入的群**。
  - **通过 `gc.approve` 审核通过的群聊，或者已经加入小群白名单的群聊，均会获得永久豁免。**
