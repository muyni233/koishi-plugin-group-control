...
#### 目标解析（approve / reject / ban / unban 通用）

这几条指令的「目标」可用三种方式指定，按优先级生效：
...
3. **自动识别**：approve/reject 对裸号会从待处理请求里自动判断是群还是好友；ban/unban 的裸号默认按**群**处理（好友请用 `friend:` 前缀或引用好友申请）。

> 若裸号同时命中待处理群邀请与好友申请，机器人会提示加前缀区分；此时用 `group:` / `friend:` 即可。
...
| `gc.friend-approve [目标]` | 同意好友申请（兼容别名 `gc.fa`，强制好友域） |
| `gc.friend-reject [目标]` | 拒绝好友申请（兼容别名 `gc.fr`，强制好友域） |
| `gc.friend-pending` | 仅查看待处理好友申请（兼容别名 `gc.fp`） |
...
| `gc.delfriend <QQ号>` | 删除指定好友（仅删除，不拉黑） |
